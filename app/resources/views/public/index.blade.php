...
<div class="filters" style="margin-bottom:28px;justify-content:center;">
    ...
    @foreach($groups as $group)
        @foreach($group->options as $option)
            @php
                $isActive = in_array($option->slug, $filterSlugs, true);
                $nextFilters = $isActive
                    ? array_values(array_diff($filterSlugs, [$option->slug]))
                    : array_values(array_unique(array_merge($filterSlugs, [$option->slug])));
                $query = count($nextFilters) ? '?' . http_build_query(['filters' => $nextFilters]) : '';
            @endphp
