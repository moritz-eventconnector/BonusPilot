...
set_env_value "DB_USERNAME" "${DB_USERNAME:-bonuspilot}" .env
set_env_value "DB_DATABASE" "${DB_DATABASE:-bonuspilot}" .env
...
docker compose exec -T app sh -c "mkdir -p storage/app/public storage/framework/cache storage/framework/sessions storage/framework/views storage/logs bootstrap/cache && chown -R www-data:www-data storage bootstrap/cache && chmod -R 775 storage bootstrap/cache"

docker compose exec -T -e PGPASSWORD="${DB_PASSWORD}" db psql -U "${DB_USERNAME}" -d postgres -v ON_ERROR_STOP=1 <<SQL
DO \$\$
BEGIN
